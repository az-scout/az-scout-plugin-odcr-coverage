# Copilot Instructions for az-scout-plugin-odcr-coverage

## Project overview

This is an **az-scout plugin** that analyses On-Demand Capacity Reservation (ODCR) coverage for Azure VMs. It identifies VMs at risk of allocation failure by combining VM inventory, Activity Log allocation events, and existing ODCR utilization.
...
```
src/az_scout_odcr_coverage/
├── __init__.py          # Plugin class + module-level `plugin` instance
├── routes.py            # FastAPI APIRouter (mounted at /plugins/odcr-coverage/)
...
odcr_coverage = "az_scout_odcr_coverage:plugin"
```
